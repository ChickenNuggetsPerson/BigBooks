        ...
        <div>
            ...
            <div className="flex justify-start gap-6 flex-col lg:flex-row">
                ...
                <AnimateChildren x={-100} y={0}>
                    ...
                    {/* New Employee Button */}
                    <Link type="button" href={`/organization/employee/new/edit`} className="w-xs card block mb-5 hover:shadow-primary">
                    ...
                    {/* Search Bar */}
                    <div className="w-full w-max-md lg:w-md mb-10 lg:mb-0 card hover:shadow-secondary">
                    ...
                    <button onClick={() => { setShowDeleted(!showDeleted) }} className={"w-full w-max-xs mb-10 lg:mb-0 card hover:shadow-accent " + (showDeleted ? "shadow-xl shadow-accent" : "")}>
                        Show Deactivated
                    </button>
                </AnimateChildren>
...
function EmployeeRow({ e, i }: { e: DispEmployee, i: number }) {
    ...
    return (
        <motion.tr onClick={() => { router.push(url) }} className="cursor-pointer rounded-lg bg-card border border-gray-200 shadow-sm mb-5 hover:bg-gray-100 "
            initial={{ opacity: 0 }}
            exit={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        >
            ...
            <td className="p-2 w-1/3 font-normal text-gray-700 ">{e.address}</td>
        </motion.tr>
    )
}