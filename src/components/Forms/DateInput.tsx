...
export default function DateInput({ 
    ...
    return (
        <div className={"relative z-0 w-full mb-5 group " + (disabled ? "opacity-50" : "")}>
            <input key={val.toISOString()} type="date" name={id} id={id} onChange={c} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" defaultValue={formatDate(val)} readOnly={disabled} />
