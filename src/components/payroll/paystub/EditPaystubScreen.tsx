...
export default function EditPaystubScreen({ period, allEmployees }: EditPaystubScreen) {
    ...
    useEffect(() => {
        ...
        setList(l)
    }, [allEmployees, period.includedEmployees])
        ...
        <div className="flex overflow-hidden" style={{ maxHeight: "calc(100dvh - 5rem)" }}>
                        ...
                        <AnimateChildren x={0} y={-10}>
                            {list.map((item) => (<>
                                <li key={item.id} onClick={() => { selectItem(item) }} className={"border-b border-secondary last:border-b-0 p-2 rounded-md hover:bg-gray-100 shadow flex flex-row"}>
                                    <div className="size-5 rounded-sm border-primary border"></div>
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden" style={{ scrollbarWidth: "none" }}>
                <AnimateChildren x={0} y={0}>
                    {selected &&
                        <PaystubForm periodUUID={period.uuid} empUUID={selected.id} />
                    }
                </AnimateChildren>
            </div>
        </div>
